<template>
	<view class="wrap">
		<view class="no_swiper" v-if="list && list.length<3" :style="{height: height *2 + 'rpx'}">
			<view class="item item_list" v-for="(item,index) in list" :key='index' @click="$emit('clickItem',item)">
				<view class="item" :class="[!crown ? '' : current==index ? 'crown-active':'crown']">
					<image v-if="!slots" class="item-img" :class="[imgShadow?'imgShadow':'']" :src="item[imgKey]"
						:style="{ borderRadius: imgRadius + 'px',width:imgWidth}" mode="heightFix"></image>
					<slot v-else :data='item'></slot>
				</view>
			</view>

		</view>
		<swiper v-else class="swiper" :style="{height: height *2 + 'rpx'}" :autoplay="autoplay" :interval="interval"
...
<script>
	export default {
		...
		methods: {
			change(event) {
				if (this.list.length < 3) return false
