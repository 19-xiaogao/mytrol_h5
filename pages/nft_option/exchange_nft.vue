<template>
  <view class="_nft_option">
    <uni-nav-bar
      left-img="https://mytrol-pub.oss-cn-shenzhen.aliyuncs.com/mytrol/icon/leftBack.png"
      left-img-h="40px"
      left-img-w="40px"
      fixed="true"
      color="#ffffff"
    ...
    <view v-show="NFT_item.file">
        ...
        <template v-slot:footer>
          ...
          >
            <view
              class="_fot_img"
              :class="[
                isGif(getIpfsSrc(NFT_item.file), null) ? '_gif_fot' : '',
              ]"
            >
              <image
                :src="isGif(getIpfsSrc(NFT_item.file), 'img_foot')"
                mode=""
              >
              </image>
            </view>
      ...
      <view class="_content">
        ...
        <view class="_user_block">
          <view class="_icon_l">
            <image
              src="https://mytrol-pub.oss-cn-shenzhen.aliyuncs.com/mytrol/icon/nftIcon.png"
              mode=""
            >
            </image>
          </view>
          ...
          <view class="_icon_l">
            <image
              src="https://mytrol-pub.oss-cn-shenzhen.aliyuncs.com/mytrol/icon/nftIconRight.png"
              mode=""
            >
            </image>
          </view>
        ...
        <view class="_creat_name">
          <view class="_left">
            ...
            <image
              src="https://mytrol-pub.oss-cn-shenzhen.aliyuncs.com/mytrol/icon/companyIcon.png"
              mode="widthFix"
              class="_hot"
            ></image>
          </view>
        ...
        <view class="_desc">
          ...
          <view class="_img" v-if="NFT_item.file_background">
            <image
              :src="getIpfsSrc(NFT_item.file_background)"
              mode="widthFix"
            ></image>
          </view>
        </view>
        ...
        <view class="_help">
          <image
            src="https://mytrol-pub.oss-cn-shenzhen.aliyuncs.com/mytrol/icon/technicalSupport.png"
            mode="heightFix"
          >
          </image>
        </view>

        <view v-if="NFT_item.publish == '2'">
          <!-- 在售 -->
          <!-- 未到开售时间 -->
          <view class="_buy" v-if="!isShowCountdown()">
            <img
              class="button-background-img"
              :src="unpublishBgsrc(NFT_item.publish)"
              alt=""
            />
            <view class="price-box">
              <view class="price">
                ...
                <view class="p2"
                  >¥{{ NFT_item.price ? NFT_item.price : "" }}</view
                >
              </view>
            ...
            <ExChangeTimeComponent
              :publishTime="NFT_item.opening_time"
              :NFT_item="NFT_item"
            ></ExChangeTimeComponent>
          </view>
          <!-- 免费白名单用户领取 -->
          ...
          >
            ...
            <view class="_time">
              <view class="_right">
                <image
                  src="https://oss.mytrol.cn/uni_mytrol/icon/buy_1.png"
                  mode="widthFix"
                  class="image"
                ></image>
                <view class="_t2" @click="$noMultipleClicks(buyNft)"
                  >立刻领取</view
                >
              </view>
            </view>
          ...
          >
            <view class="_redeem">
              ...
              <view class="_flex">
                ...
                <view
                  class="_free_btn"
                  :class="[redeem_code ? '_free_text' : '_un_free_text']"
                  @click="redeemNft()"
                >
                  立即兑换
                </view>
        ...
        <view class="_buy" v-if="NFT_item.publish == '0'">
          <img
            class="button-background-img"
            src="http://oss.mytrol.cn/uni_mytrol/img/nft_buy_bg_unpublish.png"
            alt=""
          />
          <view class="_no_publish"> 已下架 </view>
        </view>
      </view>
    </view>
  </view>
...
import ExChangeTimeComponent from "./components/exchange_time.vue";
let that;
export default {
  name: "nft_list",
  components: {
  ...
  data() {
    return {
      ...
      NFT_item: {
        ...
        desc_img:
          "https://oss.dbchain.cloud/uni_mytrol_applet/img/mock/option/nft_option.webp",
        remaining: "63",
        collected_number: "0",
      },
      // 分享
  ...
  methods: {
    ...
    isShowCountdown() {
      return new Date().getTime() > Number(this.NFT_item.opening_time) * 1000;
    },
    async getUserIsExitsWhiteList(id) {
      const result = await this.$api._get(
        "/dbchain/oracle/nft/check_user_whitelist_status/" + id
      );
      if (result.data.err_code === "0") {
        const whitelist_status = result.data.result.whitelist_status;
        const status = whitelist_status === "false" ? false : true;
        this.whitelistStatus = status;
        console.log(this.whitelistStatus);
      }
    },
    // 获取NFT IPFS地址
    getIpfsSrc(url) {
        ...
        return new Promise((resolve, reject) => {
          isOriderPoll = setInterval(async () => {
            ...
            let data = await this.$api._get(
              "/dbchain/oracle/nft/find_by/nft_publish_order/order_id/" + val
            );
            if (i > num) {
    ...
    _init_(option) {
      if (option.id) {
        this.denomId = option.id;
        this.getUserIsExitsWhiteList(option.id);
    ...
    unpublishBgsrc(item) {
      ...
      let unbuy =
        "http://oss.mytrol.cn/uni_mytrol/img/nft_buy_bg_unpublish.png";
      let isNum =
        Number(this.NFT_item.number) - Number(this.NFT_item.counter_sold_nft) >
        0;
    ...
    async redeemNft() {
      ...
      let res = await that.$api._post("/dbchain/oracle/nft/redeem_nft", params);

      if (res.data.result.free === "true") {
        this.redeem_code = "";
        return uni.reLaunch({
          url: "/pages/index/index?key=main",
        });
        ...
        let buy_data = await this.$api._post("/dbchain/oracle/nft/nft_buy", {
          pay_type: "unionpay_alipay_h5",
          vendor: "union",
          order_id: res.data.result.order_id, //"order_id"
          redirect_url: window.location.origin, // 可不填
        });
