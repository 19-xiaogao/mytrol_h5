		...
		<view class="" v-show="NFT_item.file">
			...
			<view class="_content">
				...
				<view class="_redeem" v-if="NFT_item.publish=='2'">
					...
					<view class="_flex" v-if="this.isNFTNumber()">
						<view class="input-box">
							<input type="text" v-model="redeem_code" placeholder="点击输入兑换码" placeholder-class="input_box_placeholder"/>
						</view>
						
						<view class="_free_btn" :class="[redeem_code?'_free_text':'_un_free_text']" @click="redeemNft()">
							立即兑换
						</view>
					</view>
					<view class="_no_publish" v-else>
						<view class="_free_btn" :class="[redeem_code?'_free_text':'_un_free_text']" >
							已兑完
						</view>
					</view>
				</view>
				<!-- publish 0 已下架 1 审核中 2.上架成功 3.审核失败 -->
				<!-- 已下架 -->
